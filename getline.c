 ...
 * Return: bytes read
 */
ssize_t input_buf(input_t *input, char **store, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*store = NULL;
		...
		if (r > 0)
		{
			...
			rm_comments(*store);
			history_list(input, *store, input->histcount++);
 ...
 * Return: bytes read
 */
ssize_t get_input(input_t *input)
{
	static char *store; /* the ';' command chain buffer */
	static size_t i, j, len;
	ssize_t r = 0;
	char **store_ptr = &(input->arg), *ptr;

	_putchar(BUF_FLUSH);
	r = input_buf(input, &store, &len);
	if (r == -1) /* EOF */
		return (-1);
	if (len) /* we have commands left in the chain buffer */
	{
		...
		check_chain(input, store, &j, i, len);
		while (j < len) /* iterate to semicolon or end */
		{
			if (is_chain(input, store, &j))
				break;
			j++;
		}
 ...
 * Return: r
 */
ssize_t read_buf(input_t *input, char *buf, size_t *size)
{
	ssize_t r = 0;
 ...
 * Return: s
 */
int _getline(input_t *input, char **ptr, size_t *length)
{
